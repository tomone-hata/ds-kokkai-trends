...
| 項目 | 内容 |
|---|---|
| 現在のフェーズ | **EXP-001 実施中。S-5 完了・RP-B 通過。次は S-6（ベクトル化 × クラスタリング。3日・CP-2/CP-3 を含む）** |
| 最終更新 | 2026-08-21 |
...
| 実験 | [EXP-001 plan.md](experiments/EXP-001-topic-extraction-baseline/plan.md) | **実施中**（S-5 完了 / 全11ステージ中5） | **RP-A・RP-B とも通過**（いずれも差し戻しを経て）。次のゲートは **RP-C（S-7 完了時）** |
| アルゴリズム本設計 | | | |
| システム設計 | | | |
| 実装 | | | |
| システム検証 | | | |
...
|---|---|---|---|---|
| 1 | 着手前の確定が必要な未確定事項5件（Q-013 LLMコスト／Q-015 参照リストの粒度／Q-016 一致率の算出定義／Q-017 撤退基準の閾値／**Q-018 分析単位 → S-5 で決着**） | EXP-001 の着手 | PoC設計書 | EXP-001 着手まで |
| 3 | **`poc/topic-extraction-2025` が2コミット未 push。** S-5 完了分と RP-B 対応分を含む | PR の作成 | ユーザー承認待ち | PoCフェーズ完了時 |
...
| 2026-08-21 | **RP-B が S-3〜S-5 の3決定を検証し、差し戻し1回を経て通過した。** 数値に誤りはなく再実行も不要だが、**記録が確立された内容を過大に述べていた**。SE-A の差は統計的に有意でなく（p=0.73 / 0.52）、S-4 の盲検化は設計書からの逸脱、**S-5 の U-2 脱落は実行前に確定していた**。設計上の欠陥3件を ISSUE-009・010・011 として起票し、いずれも**本フェーズでは変更せず Phase 2 の入力**とした（変更はいずれも事後の基準変更にあたるため）。**SE-B も SE-A と同構造であり、同じ問題が S-7 で再現するため RP-C の入力とした** | [RP-Bレポート](reviews/RP-B_EXP-001-S3-S5-decisions_20260821.md) |
