...
| 項目 | 内容 |
|---|---|
| 現在のフェーズ | **EXP-001 実施中。S-5 完了（U-1 採用）。次は RP-B（S-6 着手の前提）** |
| 最終更新 | 2026-08-20 |
...
| 実験 | [EXP-001 plan.md](experiments/EXP-001-topic-extraction-baseline/plan.md) | **実施中**（S-5 完了 / 全11ステージ中5） | RP-A 通過。**RP-B が未実施**（S-6 着手の前提） |
| アルゴリズム本設計 | | | |
| システム設計 | | | |
| 実装 | | | |
| システム検証 | | | |
...
| 2026-08-20 | **S-4 で E-2（≤20字を除外）を採用し、S-5 以降で固定。** 決定規則1（解釈可能率 50%）で単独首位。**E-1（T1=10）は E-0 と同率であり、S-2 で機械的に導いた T1 は解釈可能率の観点では最適でなかった。** 手順の見直しは事後の基準変更にあたるため行わず、Phase 2 への申し送りとした（PoC はベンチマーク作りという位置づけによる） | [config.md](../experiments/EXP-001-topic-extraction-baseline/config.md) |
| 2026-08-20 | **S-5 で U-1（発言単位）を採用。** 決定規則1（D-3 該当の除外）で決着し、**U-2 は D-3b（過分散）に該当して除外**された（100クラスタ ÷ 1,126会議 = 8.881%）。**規則2 の主観評価には進んでいない。** ISSUE-001 で要件定義書 v1.0 の「発言単位」の根拠が飛躍と指摘され比較項目へ格上げされた争点が、**実験による判定で決着した** | [config.md](../experiments/EXP-001-topic-extraction-baseline/config.md) |
