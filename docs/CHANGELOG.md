...
| 対象 | 変更内容 | 関連 |
|---|---|---|
| PoC設計 | **v1.1へ改訂。** レビューが「当該ステージ到達までに解消」とした3件のうち、EXP-001 の `plan.md` 作成前に必要な分を解消。**構成の確定ステージを S-7 に明示**（9.4節 規則2 が SE-B を用いるため S-7 でしか確定できない）、**9.4節 規則2 の主観評価を SE-B に特定**、**2.4節の D-3 を「撤退判定には用いない／選定のための判定には用いてよい」と書き分け**。未処理の指摘は10件から7件へ | [ISSUE-003](issues/ISSUE-003-poc-design-stage-and-decision-rules/issue.md) |
