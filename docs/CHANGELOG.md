...
| 対象 | 変更内容 | 関連 |
|---|---|---|
| PoC設計 | **v1.0 から v1.2 へ改訂。** (1) 構成の確定ステージを S-7 に明示（9.4節 規則2 が SE-B を用いるため S-7 でしか確定できない）、9.4節 規則2 の主観評価を SE-B に特定、2.4節の D-3 を「撤退判定には用いない／選定のための判定には用いてよい」と書き分け。未処理の指摘は10件から7件へ。(2) **H-009 を「論点の推移が可視化に耐える粒度で得られる」から「時系列集計が動作し、会期・委員会構成という既知の構造を再現できる」へ改定。** 11章に交絡（比率化では解決しない。分母の委員会構成自体が入れ替わる）を明記し、**TM-4・TM-5 を追加**。年内の時系列は論点のトレンドではなく**動作確認の対照実験**と位置づけた | [ISSUE-003](issues/ISSUE-003-poc-design-stage-and-decision-rules/issue.md) / [ISSUE-005](issues/ISSUE-005-timeseries-scope-and-committee-confound/issue.md) |
